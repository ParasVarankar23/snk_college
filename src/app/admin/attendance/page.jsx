...
export default function AdminAttendancePage() {
    ...
    const subjectOptions = useMemo(() => {
        return getDepartmentSubjects(department);
    }, [department]);
    ...
    const loadData = async () => {
        try {
            ...
            const [studentsResponse, attendanceResponse] = await Promise.all([
                fetch(`/api/attendance?type=students&department=${encodeURIComponent(department)}`, {
                    headers: { Authorization: `Bearer ${token}` },
                    cache: "no-store",
                }),
                fetch(
                    `/api/attendance?date=${encodeURIComponent(date)}&department=${encodeURIComponent(department)}&subject=${encodeURIComponent(subject)}`,
                    {
                        headers: { Authorization: `Bearer ${token}` },
                        cache: "no-store",
                    }
    ...
    if (loading) {
        ...
        attendanceRows = paginatedStudents.map((student) => (
            <tr key={student.id} className="border-t border-stone-100">
                ...
                <td className="px-3 py-2 capitalize">{student.department}</td>
    ...
    return (
        <div className="min-h-screen space-y-6 bg-[#f6f3f1] p-4 md:p-6">
            <div className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
                ...
                <p className="mt-1 text-sm text-slate-600">Manage attendance for all departments.</p>
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                    <Stat label="Present" value={attendanceStats.present || 0} />
                    <Stat label="Absent" value={attendanceStats.absent || 0} />
                    <Stat label="Leave" value={attendanceStats.leave || 0} />
                </div>
            </div>
