...
export default function ScienceTeachersSection() {
    ...
    useEffect(() => {
        const fetchTeachers = async () => {
            try {
                const response = await fetch("/api/departments?department=science", {
                    cache: "no-store",
                });
                const data = await response.json();

                if (!response.ok) {
                    throw new Error(data.error || "Failed to load science teachers");
                }

                setTeachers(data.teachers || []);
            } catch (err) {
                setError(err.message || "Failed to load science teachers");
            } finally {
                setLoading(false);
            }
        };
        ...
        <section className="bg-gray-50 py-10 min-h-[60vh]">
            <div className="max-w-7xl mx-auto px-6">
                <div className="text-center mb-14">
                    <h2 className="text-3xl font-bold text-gray-800">Science Department Teachers</h2>
                ...
                {!loading && !error && teachers.length === 0 && (
                    <p className="text-center text-gray-500">No teachers added yet for Science department.</p>
                )}
